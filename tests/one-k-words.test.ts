...
test('index page has expected h1', async ({ page }) => {
	await expect(
		page.getByRole('heading', { name: '1000 Japanese basic words', level: 1 })
	).toBeVisible();
});
...
test('index page has expected meta description', async ({ page }) => {
	...
	await expect(metaDescription).toHaveAttribute(
		'content',
		'One Thousand Words - Learn Japanese words with flashcards.'
	);
});
...
test('index page has expected meta twitter', async ({ page }) => {
	...
	await expect(metaTwitterTitle).toHaveAttribute(
		'content',
		'One Thousand Words - Nihongo Flashcard'
	);
